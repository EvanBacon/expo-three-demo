...
class Passes {
	constructor(presetLabel) {
		...
		this.hBlurTween = new THREEx.UniformsTween(hBlurPass.uniforms, {
		...
		// reset of all values
		THREEx.ToxicPproc.passesPreset['sober'].init(this);
		// init current preset
		preset.init(this);
...
THREEx.ToxicPproc = {
	...
	passesPreset: {
		sober: {
			init: function (self) {
				// hBlurPass
				var uniforms = self.hBlurTween.dstUniforms || { h: {} };
				uniforms.h = uniforms.h || {};

				uniforms.h.value = 0;

				// vBlurPass
				var uniforms = self.vBlurTween.dstUniforms || { v: {} };
				uniforms.v = uniforms.v || {};
				uniforms.v.value = 0;

				// rgbRadialPass
				var uniforms = self.rgbRadialTween.dstUniforms || { factor: {}, power: {} };
				uniforms.factor = uniforms.factor || {};
				uniforms.power = uniforms.power || {};

				uniforms.factor.value = 0;
				uniforms.power.value = 3.0;

				// seeDoublePass
				var uniforms = self.seeDoubleTween.dstUniforms || { radius: {}, timeSpeed: {}, mixRatio: {}, opacity: {} };
				uniforms.radius = uniforms.radius || {};
				uniforms.timeSpeed = uniforms.timeSpeed || {};
				uniforms.mixRatio = uniforms.mixRatio || {};
				uniforms.opacity = uniforms.opacity || {};
				...
				// refractionPass
				var uniforms = self.refractionTween.dstUniforms || { timeSpeed: {}, Frequency: {}, Amplitude: {} };
				uniforms.timeSpeed = uniforms.timeSpeed || {};
				uniforms.Frequency = uniforms.Frequency || {};
				uniforms.Amplitude = uniforms.Amplitude || {};
